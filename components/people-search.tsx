...
export default function PeopleSearch({ users, departments, currentUser }) {
  ...
  useEffect(() => {
    if (users?.length && currentUser?.user?.role) {
      if (currentUser.user.role === "EMPLOYEE") {
        const deptFiltered = users.filter(
          (user) => user.departmentId === currentUser.user.departmentId
        );
        setBaseUsers(deptFiltered);
        setFilteredUsers(deptFiltered);
      } else {
        setBaseUsers(users);
        ...
        <TabsContent value="departments">
          <div className="space-y-6 dark:bg-gray-900">
            {(currentUser.user.role === "EMPLOYEE"
              ? departments?.filter(
                (d) => d.id === currentUser.user.departmentId
              )
              : departments
            )?.map((department) => {
