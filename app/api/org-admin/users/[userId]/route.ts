...
export async function PATCH(req: Request, { params }: { params: { userId: string } }) {
  try {
    ...
    const updated = await db.user.update({ where: { id: params.userId }, data: { name: name || undefined, role: role || undefined }, select: { id: true, name: true, email: true, role: true } })
...
export async function POST(req: Request, { params }: { params: { userId: string } }) {
  try {
    ...
    const updated = await db.user.update({ where: { id: params.userId }, data: { name: name as any, role: role as any }, select: { id: true, name: true, email: true, role: true } })
    return NextResponse.json(updated)
  } catch (error: any) {
